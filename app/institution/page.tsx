    ...
    <CardContent className="p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          {detail && <p className="text-xs text-green-600">{detail}</p>}
        </div>
...
export default function InstitutionPortal() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true);
        const [statsRes, certsRes, logsRes, analyticsRes] = await Promise.all([
  ...
  const handleSingleIssueSubmit = async (e: FormEvent) => {
    ...
    try {
      const response = await fetch(`${baseUrl}/api/issue-certificate`, {
        method: "POST",
      ...
      const certsRes = await fetch("/api/dashboard?action=recent-certificates");
      setRecentCertificates(await certsRes.json());
    } catch (error) {
      console.error(error);
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">
            Manage certificate issuance and track verification analytics
          </p>
        </div>
        ...
        <Tabs defaultValue="dashboard" className="space-y-6">
            ...
            <Card className="glass-card">
              ...
              <CardContent className="space-y-6">
                ...
                <div className="flex flex-col gap-2">
                  {uploadedCertErrors &&
                    uploadedCertErrors.map((error, index) => (
                      <div
                        key={index}
          ...
          <TabsContent value="single-issue">
            <Card className="glass-card">
              ...
              <CardContent className="space-y-6">
                <form onSubmit={handleSingleIssueSubmit}>
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      <Label htmlFor="courseName">Program/Course</Label>
                      <Select
                    ...
                    <div>
                      <Label htmlFor="insstitutionName">Institution Name</Label>
                      <Input
                        id="institution"
                ...
                <Table>
                  ...
                  <TableBody>
                    {analyticsData?.programStats &&
                      analyticsData?.programStats?.length > 0 &&
